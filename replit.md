...
- **Solution Profile with Vendor Categories**: Solution profile pages (/solution/<id>) now feature Active and Passive vendor categories displayed in separate sections. Each section has its own styled "Add Vendor" button (green for Active, purple for Passive). Vendor cards support logo images (via URL) with graceful fallback to initials. Database includes vendor_category column with automatic migration for existing tables. Only M.Saleh can add/edit/delete vendors.
