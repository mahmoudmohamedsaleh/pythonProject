...
- **Sales & Presales Tools**: RFQ management (with comments and notifications), quotation builder, solution builder, cost sheet management, product catalog (fire alarm, CCTV, passive), and an AI-Powered CCTV Smart Selector for intelligent product recommendations, AI-Generated Executive Summary in quotation PowerPoint presentations (using OpenAI via Replit AI Integrations), and a Quotation Price Checker tool. Includes **Cost Sheet Builder** with save/load per RFQ+quote reference (stored in `cost_sheets` SQLite table), Import/Export Excel, bidirectional Margin↔Unit Price recalculation, insert-before/after rows, and section headers.
...
- **Project Implementation**: Post-sale project delivery management with transfer from Closed Won projects, team assignment (Project Manager, Site Engineer, Project Coordinator), milestone tracking with default phases (Design & Planning, Procurement, Installation, Testing & Commissioning, Handover & Closeout), progress calculation, activity logging, permission-based access control allowing assigned team members to view their projects, and **Milestone Task Management** with clickable phase names leading to dedicated task pages supporting full CRUD (add/edit/delete tasks), status tracking (Pending, In Progress, Completed, On Hold, Cancelled), priority levels (Low, Medium, High, Urgent), user assignment, due dates, follow-up notes, and filtering.
