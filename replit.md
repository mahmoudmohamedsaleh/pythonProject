...
- **Active Users Tracking**: Admin-only (General Manager) page showing currently logged-in users in real-time. Features include session tracking via database table (active_sessions), login/logout recording, last activity timestamp updates on each request, user status indicators (Online/Idle/Away based on activity time), session termination capability, auto-refresh every 30 seconds, KPI cards showing active count, total users, and online rate percentage, device/browser detection via user-agent, IP address logging, and automatic stale session cleanup (24-hour timeout).
