...
- **Solution Profile with Vendor Categories**: Solution profile pages (/solution/<id>) support vendor management. ICT Infrastructure solution exclusively features Active and Passive vendor categories displayed in separate sections with styled buttons (green for Active, purple for Passive). All other solutions show standard single vendor section. Vendor cards support logo images (via URL) with graceful fallback to initials. Database includes vendor_category column with automatic migration for existing tables. Only M.Saleh can add/edit/delete vendors.
